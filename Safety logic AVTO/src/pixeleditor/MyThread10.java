...
public class MyThread10 implements Runnable {
	...
	public MyThread10(GameModel model, MdfBufferStorage mbs,  Cell c1) throws Exception {
		this.model = model;
...
//	}
	...
	public MyThread10(GameModel gameModel, MdfBufferStorage mbs2, Cell c1, Cell c2) {
	 ...
	 public void run()
     {
		 try
		 {
			...
			while( (!Thread.currentThread().isInterrupted())  )
			{
				...
				int mode = HelperRandom.getRandomInt(0, 20);
				if(mode<10) {
				...
				if(cc==null) {
					...
					ArrayList<Cell> ncells = model.defineNeigh(this.cell.x, this.cell.y, this.cell.getMyColor());
					ArrayList<int[]> colors = new ArrayList<int[]>(); 
					for(Cell tcell: ncells) {
						//int[] ti = tcell.getMyColor();
						//String cl = ""+ti[0]+","+ti[1]+","+ti[2]+","+ti[3];
						boolean found=false;
						for(int[] iti: colors) {
							if(tcell.compareColor(iti)) {found=true; break;}
						}
						if(found==false) {
							colors.add(tcell.getMyColor());
						}
					...
					for(int[] clr: colors) {
						...
						int conter=0;
						for(Cell tcell: ncells) {
							if(tcell.compareColor(clr)) {
								conter++;
							}
						}
						if(max<conter) { max=conter; maxcolor = clr; }
					}
				...
				Cell[] cc2 = this.getMinMaxClusterSize(maxcolor);
				if(cc2==null) {
					this.cell = null;
			 		this.cell2 = null;
					continue;
				...
				if(this.cell == null || this.cell2==null) {
					this.cell = null;
			 		this.cell2 = null;
					continue;
				}
...
///////////////////////////////////////////////////////// works //////////////////////////////////
				...
				ArrayList<Cell> sames =  mdf.getNeighboursSameColor(model.cells, cell);
				if(sames.size()>2)  {
					this.cell = null;
			 		this.cell2 = null;
					continue;
...
//				
//				ArrayList<Cell> sames2 =  mdf.getNeighboursSameColor(model.cells, cell2);
//				if(sames2.size()>2)  {
//					this.cell = null;
//			 		this.cell2 = null;
//					continue;
...
//				Cell t = new Cell(cell2);
//				t.setColor(cell.getMyColor());
//				ArrayList<Cell> sames3 =  mdf.getNeighboursSameColor(model.cells, t);
//				if(sames3.size()>sames.size())  {
//					this.cell = null;
//			 		this.cell2 = null;
//					continue;
//				}
